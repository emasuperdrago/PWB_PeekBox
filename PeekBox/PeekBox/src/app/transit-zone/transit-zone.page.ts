...
export class TransitZonePage implements OnInit, OnDestroy {
  ...
  // ★ FIX BUG 1: riceve il valore direttamente da $event.detail.value (ionChange)
  //   invece di leggerlo da this.boxSorgenteId (che in Ionic 8 con [(ngModel)] arriva in ritardo)
  onBoxSorgenteChange(nuovoId: number) {
    this.boxSorgenteId = Number(nuovoId);
    console.log('[transit] boxSorgenteId impostato a:', this.boxSorgenteId);
  ...
  // ★ FIX BUG 1: stessa strategia per la box destinazione
  onBoxDestinazioneChange(nuovoId: number) {
    this.boxDestinazioneId = Number(nuovoId);
    console.log('[transit] boxDestinazioneId impostato a:', this.boxDestinazioneId);
