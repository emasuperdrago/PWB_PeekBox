...
export class DatabaseService {
  ...
  private getAuthHeaders(): HttpHeaders {
    ...
    if (!token) {
      // ★ FIX A: token assente → log esplicito invece di mandare "Bearer null"
      console.error('[DatabaseService] Token JWT assente in localStorage. Utente non autenticato.');
    }
   ...
   * ★ FIX: validazione esplicita dei parametri prima della chiamata HTTP
   *        per evitare UPDATE silenzioso con ID null/undefined/0.
   */
  spostaOggetti(oggettiIds: number[], boxDestinazioneId: number) {
    // ★ FIX A: filtra eventuali ID non validi prima di inviare
    const idsPuliti = oggettiIds.map(Number).filter(id => !isNaN(id) && id > 0);
    const destId    = Number(boxDestinazioneId);

    if (idsPuliti.length === 0) {
      console.error('[spostaOggetti] Nessun ID oggetto valido da spostare:', oggettiIds);
      throw new Error('ID oggetti non validi');
    }
    if (!destId || isNaN(destId) || destId <= 0) {
      console.error('[spostaOggetti] ID box destinazione non valido:', boxDestinazioneId);
      throw new Error('ID box destinazione non valido');
    }

    console.log(`[spostaOggetti] Invio PUT — ids: [${idsPuliti}] → box ${destId}`);
